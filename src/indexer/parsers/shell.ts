...
// Patterns to detect Shell/Bash declarations
const PATTERNS = [
  // Function definitions
  /^(?:function\s+)?(\w+)\s*\(\)\s*\{?/,
  // Variable assignments (exported or important ones)
  /^export\s+(\w+)=/,
  /^readonly\s+(\w+)=/,
  /^declare\s+(?:-[a-zA-Z]+\s+)?(\w+)=/,
];
...
function getDeclarationType(line: string): string {
  if (/\(\)/.test(line)) return 'function';
  if (/^export\b/.test(line)) return 'export';
  if (/^readonly\b/.test(line)) return 'readonly';
  if (/^declare\b/.test(line)) return 'declare';
  return 'function';
}

export function parseShell(content: string, _filePath: string): Section[] {
